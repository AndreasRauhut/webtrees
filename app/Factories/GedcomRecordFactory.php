 ...
 */
class GedcomRecordFactory extends AbstractGedcomRecordFactory
{
    /**
    ...
    public function make(string $xref, Tree $tree, string $gedcom = null): ?GedcomRecord
    {
        ...
        return
            app(FamilyFactory::class)->make($xref, $tree, $gedcom) ??
            app(IndividualFactory::class)->make($xref, $tree, $gedcom) ??
            app(MediaFactory::class)->make($xref, $tree, $gedcom) ??
            app(NoteFactory::class)->make($xref, $tree, $gedcom) ??
            app(RepositoryFactory::class)->make($xref, $tree, $gedcom) ??
            app(SourceFactory::class)->make($xref, $tree, $gedcom) ??
            app(SubmitterFactory::class)->make($xref, $tree, $gedcom) ??
    ...
    protected function newGedcomRecord(string $type, string $xref, string $gedcom, ?string $pending, Tree $tree): ?GedcomRecord
    {
        switch ($type) {
            case Family::RECORD_TYPE:
                return app(FamilyFactory::class)->new($xref, $gedcom, $pending, $tree);

            case Individual::RECORD_TYPE:
                return app(IndividualFactory::class)->new($xref, $gedcom, $pending, $tree);

            case Media::RECORD_TYPE:
                return app(MediaFactory::class)->new($xref, $gedcom, $pending, $tree);

            case Note::RECORD_TYPE:
                return app(NoteFactory::class)->new($xref, $gedcom, $pending, $tree);

            case Repository::RECORD_TYPE:
                return app(RepositoryFactory::class)->new($xref, $gedcom, $pending, $tree);

            case Source::RECORD_TYPE:
                return app(SourceFactory::class)->new($xref, $gedcom, $pending, $tree);

            case Submitter::RECORD_TYPE:
                return app(SubmitterFactory::class)->new($xref, $gedcom, $pending, $tree);
